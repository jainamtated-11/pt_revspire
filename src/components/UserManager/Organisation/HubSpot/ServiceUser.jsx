...
const ServiceUser = () => {
  ...
  const handleReconnect = async () => {
    const HubspotAuthorization = new Promise(async (resolve, reject) => {
      try {
        ...
        if (authData.authUrl) {
          ...
          if (authResponse.data == "HubSpot data synchronized successfully.") {
    ...
    toast.promise(HubspotAuthorization, {
  ...
  return (
    <div className="p-4 m-2 border rounded-b-lg shadow-md">
      ...
      {loading ? (
      ...
      ) : !error && serviceUserData && serviceUserData[0].crm == "Hubspot" ? (
        ...
        <>
          <div className="pb-4">
            Existing Service User is from different CRM other than HubSpot.{" "}
            <br />
            Connect New to use HubSpot as Service User.
          </div>
        </>
      )}
      {showConnectServiceUserDialog && (
