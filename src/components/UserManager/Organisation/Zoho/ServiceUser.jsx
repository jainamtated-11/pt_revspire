...
const ServiceUser = () => {
  ...
  const handleReconnect = async () => {
    const zohoAuthorization = new Promise(async (resolve, reject) => {
      try {
        ...
        const authResponse = await axiosInstance.post("/zoho-authorization", {
  ...
  return (
    <div className="p-4 m-2 border rounded-b-lg shadow-md">
      ...
      {loading ? (
      ...
      ) : !error && serviceUserData && serviceUserData[0].crm == "Zoho" ? (
        ...
        <>
         <div className="pb-4">
         Existing Service User is from different CRM other than Zoho. <br />
         Connect New to use Zoho as Service User.
         </div>
        </>
      )}
      {showConnectServiceUserDialog && (
