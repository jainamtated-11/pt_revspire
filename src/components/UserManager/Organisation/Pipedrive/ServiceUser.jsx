...
const ServiceUser = () => {
  ...
  const handleReconnect = async () => {
    const PipedriveAuthorization = new Promise(async (resolve, reject) => {
      try {
        ...
        const authResponse = await axiosInstance.post(
          ...
          {
            crm_connection_id: serviceUserData[0].id,
            viewer_id: viewer_id,
            originURL: path,
          }
        );
        ...
        if (authData.authUrl) {
          ...
          if (authResponse.data == "Pipedrive data synchronized successfully.") {
    ...
    toast.promise(PipedriveAuthorization, {
  ...
  return (
    <div className="p-4 m-2 border rounded-b-lg shadow-md">
      {disconnectDialog && (
        <WarningDialog
      ...
      {connectDialog && (
        <WarningDialog
          title="Connect Service User?"
          content="Are you sure you want to Connect New Service User? This will Disconnect your existing Service CRM, if any."
          onConfirm={handleConnect}
          onCancel={() => setConnectDialog(false)}
            ...
            {dropdownOpen && (
              ...
              >
                ...
                >
                  {!error &&
                    serviceUserData &&
                    serviceUserData[0].crm == "Pipedrive" && (
                      <>
                        <li>
                          <button
                            onClick={() => {
      ...
      {loading ? (
        <div className="flex flex-col gap-2">
          <div className="w-[200px] h-[15px] bg-neutral-200 animate-pulse rounded-lg" />
          <div className="max-w-[500px] w-full h-[15px] bg-neutral-200 animate-pulse rounded-lg" />
          <div className="max-w-[500px] w-full h-[15px] bg-neutral-200 animate-pulse rounded-lg" />
        </div>
      ) : !error && serviceUserData && serviceUserData[0].crm == "Pipedrive" ? (
        ...
        <>
          <div className="pb-4">
            Existing Service User is from different CRM other than Pipedrive.{" "}
            <br />
            Connect New to use Pipedrive as Service User.
          </div>
        </>
      )}
      {showConnectServiceUserDialog && (
