...
function HighlightAndTeams() {
  ...
  const handleVideoDelete = (index) => {
    if (!pitchState.highlightVideos[index].file) {
      dispatch(
        setDeleteHightlightVideoIds(pitchState.highlightVideos[index].id)
      );
    }
  ...
  const fetchVideoData = async (videoId, index) => {
    ...
    const fetchPromise = axiosInstance.post(`/pitch-preview-content`, {
  ...
  const handleVideoEdit = (index) => {
    const videoUrl = pitchState.highlightVideos[index]?.url;
    const videoId = pitchState.highlightVideos[index]?.id;
  ...
  return (
    <div className="h-[98%] overflow-y-auto p-2">
      ...
      <div className="">
        ...
        {showPreview &&
          (pitchState.highlightVideos[previewIndex]?.file || previewUrl) && (
            <HighlightPreview
