...
import { FaTrash, FaPlus, FaUserPlus, FaAt, FaUser } from "react-icons/fa";
...
const ContactManager = ({ setContacts }) => {
  ...
  const handleAddRow = () => {
    ...
    dispatch(toggleContactSelection(newRow));
  ...
  const handleSelectContact = (id) => {
    const contact = pitchState.allContacts.find((contact) => contact.id === id);
    dispatch(toggleContactSelection(contact));

    if (pitchState.selectedContacts.some((selected) => selected.id === id)) {
      setContacts(
        pitchState.selectedContacts.filter((selected) => selected.id !== id)
      );
    } else {
  ...
  const handleDeleteRow = (id) => {
    ...
    dispatch(setSelectedContacts(updatedSelectedContacts));
