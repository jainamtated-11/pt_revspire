...
function SectionsAndContents() {
  ...
  //States for Process Overview Modal
  ...
  const showAIGenerateButton = checkUserLicense("Revenue Enablement Spark");
  ...
  const toggleContentSelection = (sectionIndex, contentIndex, content) => {
    ...
    const key = `${sectionIndex}-${contentId}-${contentIndex}`;
  ...
  const createGroup = () => {
    ...
    if (
      ...
      selectedContentsForGroup.forEach((item) => {
        dispatch(
          assignContentToGroup({
  ...
  const handleRemoveContent = async (sectionIndex, contentIndex, content) => {
    // Case 1: Not a pitch feature - always remove from section
    if (content?.mimetype !== "/pitchfeature") {
      dispatch(removeContentFromSection({ sectionIndex, contentIndex }));
      return;
    }

    // Case 2: Pitch feature - try to delete from backend first
    try {
      const response = await axiosInstance.post(
        "/pitch-content-feature/delete-feature",
        {
          content_id: content.content_id,
        }
      );

      // Only remove from section if API deletion was successful
      if (response.data.message === "Feature deleted successfully.") {
        dispatch(removeContentFromSection({ sectionIndex, contentIndex }));
        toast.success("Process overview deleted successfully");
      } else {
        toast.warning("Failed to delete process overview");
        // Don't dispatch removeContentFromSection on failure
      }
    } catch (error) {
      console.error("Delete feature error:", error);
      toast.error(
        error.response?.data?.message || "Error deleting process overview"
      );
      // Don't dispatch removeContentFromSection on error
    }
  };
  ...
  // Updated TaglineForm component (keep this the same as before)
  const TaglineForm = ({
    sectionIndex,
    contentIndex,
    content,
  ...
  // Render function for a single content item
        ...
        {(provided, snapshot) => (
          ...
          >
            <div className="flex items-center gap-2 flex-grow min-w-0 ml-2 mr-2 ">
              ...
              <button
                onClick={() => {
                  handleRemoveContent(sectionIndex, originalIndex, content);
                }}
  ...
  // useDropdownPosition hook (copied from EditPitchV2)
    ...
    <div ref={containerRef} className="h-[100%] overflow-y-auto p-2">
      ...
      {actionPlaneModalOpen && (
        <ActionPlanEditor
          ...
          hexColor={pitchState.primaryColor || pitchState.orgColor}
          onClickHandler={onMultiSelectHandler}
          contentWhileEditing={selectedContent}
      ...
      <div className="flex flex-col h-full">
        <div className="overflow-y-auto flex-grow ">
          ...
          <div className="flex gap-2 mb-2">
            ...
            {showAIGenerateButton == 1 && <AIGenerateButton />}
          </div>
                  ...
                  {pitchState.sections.map((section, index) => {
                    ...
                    groups.forEach((group) => {
                      ...
                      while (
                        ...
                        ungrouped[currentUngroupedIndex].arrangement <
                          group.arrangement
                      ) {
                        const content = ungrouped[currentUngroupedIndex];
                        renderedItems.push(
                          renderContentItem(
                            content,
                            index,
                    ...
                    while (currentUngroupedIndex < ungrouped.length) {
                      const content = ungrouped[currentUngroupedIndex];
                      renderedItems.push(
                        renderContentItem(
                          content,
                                  ...
                                  {addMenuOpenSectionIndex === index && (
                                    <div className=" add-menu-dropdown absolute right-0 mt-2 z-20 bg-white border border-gray-200 rounded shadow-lg min-w-[180px] p-1">
                                      ...
                                      {pitchState.actionPlan && (
                                        ...
                                        >
                                          <SiGithubactions className="mr-2" />
                                          Action Plan
                                        </button>
                                      )}
            ...
            >
              <div className="flex items-center gap-2 flex-grow min-w-0 ml-2">
                <span className="flex-shrink-0 font-semibold whitespace-nowrap ">
                  {`Section ${pitchState.sections.length + 1} Title : `}
                </span>
                <input
                  type="text"
