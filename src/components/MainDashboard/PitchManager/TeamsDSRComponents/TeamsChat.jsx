...
const TeamsChat = ({ 
  ...
  const initWebSocket = useCallback(() => {
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        if (data.type === 'teams_message_update' && data.chatId === chatId) {
          setMessages(prev => {
            const messageMap = new Map();
            prev.forEach(msg => messageMap.set(msg.id || `${msg.timestamp}-${msg.text}`, msg));
            
            const newMessages = (data.conversationData?.messages || [])
              .filter(msg => msg.sender !== 'external');
    ...
    wsRef.current.onclose = (event) => {
      ...
      // Auto-reconnect logic (only if not manually closed)
  ...
  }, [chatId, baseURL, ownerName, sendRefreshPayload]);
  ...
  const sendMessage = async () => {
    ...
    try {
      await axiosInstance.post('/microsoft-teams/continue-chat', {
        chatId,
        message: messageText,
  ...
  const handleMinimize = () => {
    // Don't close WebSocket, just minimize
    setIsMinimized(true);
  };
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-full max-w-xs sm:max-w-sm bg-gray-300 rounded-lg shadow-xl flex flex-col border border-gray-200">
      {/* Header */}
      <div className="p-3 bg-gradient-to-r from-[#6264A7] to-[#464775] flex items-center rounded-lg justify-between">
      ...
      >      
        {isLoading ? (
          <div className="flex items-center justify-center h-full">
            <div className="animate-spin rounded-full h-5 w-5 border-2 border-[#6264A7] border-t-transparent"></div>
          </div>
        ) : messages.length === 0 ? (
          ...
          messages.map((msg, i) => (
            ...
            >
              <div 
                className={`max-w-[80%] p-2 rounded-lg text-sm ${msg.sender === 'external' 
                  ? 'bg-[#6264A7] text-white rounded-br-none' 
                  : 'bg-white border border-gray-200 rounded-bl-none'}`}
              >
                <div className="whitespace-pre-wrap">{msg.text}</div>
                <div className={`flex items-center justify-end mt-1 space-x-1 text-xs ${msg.sender === 'external' ? 'text-white/80' : 'text-gray-500'}`}>
                  <span>
        ...
        <div className="flex items-center space-x-2">
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-1 focus:ring-[#6264A7] resize-none disabled:bg-gray-100"
