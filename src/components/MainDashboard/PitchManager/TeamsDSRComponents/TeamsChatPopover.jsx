...
const TeamsChatPopover = ({ 
  ...
  const fetchExistingChats = useCallback(async () => {
    if (hasActiveSession) return; // Don't fetch if there's an active session
    
    try {
      setIsLoadingChats(true);
      const response = await axiosInstance.post('/microsoft-teams/get-all-chats', {
        external_user_email: email,
        manual_token: token,
      });
      if (response.data && Array.isArray(response.data)) {
        setExistingChats(response.data.map(chat => ({
          ...chat,
          conversation_data: typeof chat.conversation_data === 'string' 
            ? JSON.parse(chat.conversation_data)
  ...
  const handleStartChat = async () => {
    ...
    try {
      const response = await axiosInstance.post('/microsoft-teams/start-new-chat', {
        external_user_email: email,
  ...
  const handleCloseChat = () => {
    ...
    setTimeout(() => {
      ...
      setShowConnectDialog(true);
      setIsTransitioning(false);
      fetchExistingChats(); // Refresh the list when returning
    }, 300);
  };
  ...
  if (!isTeamsInstalled) return null;
        ...
        <div className="fixed bottom-4 right-4 z-50 w-full max-w-xs sm:max-w-sm bg-white rounded-lg shadow-xl border border-gray-200">
                ...
                <div className="mt-2 border border-gray-100 rounded max-h-40 overflow-y-auto">
                  {isLoadingChats ? (
                    <div className="p-3 flex justify-center">
                      <div className="animate-spin rounded-full h-4 w-4 border-2 border-[#6264A7] border-t-transparent"></div>
                    </div>
                    ...
                    existingChats.map(chat => (
                      ...
                      >
                        <div className="flex justify-between">
                          <div>
                            {new Date(chat.created_at || chat.last_activity).toLocaleDateString('en-US', {
                              month: 'short',
                              day: 'numeric'
                            })}
                          </div>
          ...
          <div className="bg-gray-50 px-4 py-3 flex justify-between items-center border-t border-gray-200">
            ...
            <button
              ...
              className="bg-[#6264A7] text-white px-3 py-1.5 rounded text-xs font-medium flex items-center space-x-1 disabled:opacity-50"
            >
              {isConnecting ? (
                <>
...
export default TeamsChatPopover;